...
test_that("calc_mutation_frequencies() adds correct CCF column", {
  ...
  object <- init_cevodata("test", snvs = snvs, cnas = cnas) |>
    add_metadata(purities)
  ...
  expect_equal(res_snvs, expected)
  expect_equal(get_snvs_frequency_measure(res_snvs), "CCF/2")
})
